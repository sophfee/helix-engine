...
#include "gpu/backends/dx12_enums.hpp"
#include "gpu/backends/detail/vulkan_enum_conversion.hpp"
//#define DDS_LOADER_DEBUG

#ifdef DDS_LOADER_DEBUG
#define DDS_DebugPrint(fmt, ...) helix_print("DDS: " fmt, __VA_ARGS__)
#else
...
Error DDS_UploadFromStdIO(FILE *file, std::shared_ptr<RID> texture_rid, std::string &error) {
	...
	RID buffer = driver->create_buffer(desc);
	LPBYTE buffer_data = static_cast<LPBYTE>(driver->get_mapped_data(buffer));
	...
	if (hash(ddsFileHeaderInfo->dwFourCC) == hash("DX10")) {
		...
		gfxFormat.dwFormat = vk::detail::revert(DxgiFormatToVulkan(dds.header10.dxgiFormat));
		gfxFormat.dwBlockSize = 16;
	}
	...
	*texture_rid = driver->create_image(imageDescriptor);
