...
namespace detail {
	...
	constexpr _STD uint32_t crc32_string(_STD string_view const szStr) noexcept {
		return crc32(szStr.data(), szStr.length()) ^ 0xFFFFFFFF;
	}
}

constexpr _STD uint32_t hash(_STD string_view const szStr) noexcept {
	return detail::crc32_string(szStr);
}
